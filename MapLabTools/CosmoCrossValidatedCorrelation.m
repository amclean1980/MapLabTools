classdef CosmoCrossValidatedCorrelation < CosmoModule
  
  properties (Access = public)
    
    %options
    template = [];                % contrast template.  default is diagonal vs non-diagonal
    partitionType = 'nfold'       % we're doing split half so don't change.
  ...
  methods (Access = public)
    ...
    function obj = CosmoCrossValidatedCorrelation(id, pn)
    ...
    function mat = getCorrelationMatrix(obj)
      mat = obj.corrMatrix;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function obj = execute(obj)
      ...
      for m = 1:length(obj.maskFileNames)
        ...
        % setup paritioning
        if strcmp(obj.partitionType, 'half')
          obj.partitionType = cosmo_nchoosek_partitioner(obj.ds{m},'half');
        elseif strcmp(obj.partitionType, 'nfold')
          obj.partitionType = cosmo_nfold_partitioner(obj.ds{m});
        end
